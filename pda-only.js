...
// Connect to devnet (regular endpoint)
const connection = new Connection('https://api.devnet.solana.com', 'confirmed');

// Get the exact seeds used in the contract to derive the PDA
async function createRaffle() {
  try {
    ...
    // Step 1: Get the current timestamp from blockchain
    console.log("\n-> Fetching current blockchain time...");
    ...
    // Step 3: Convert timestamp to little-endian bytes (exactly like Rust)
...
createRaffle()
  .then(success => {
    if (success) {
      console.log("\n=== PDA DERIVATION TEST SUCCESSFUL! ===");
