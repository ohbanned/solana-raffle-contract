...
// Connect to devnet with higher commitment and preflight checks disabled
...
async function initializeConfig() {
  ...
  if (existingConfig) {
    console.log("Config already exists, no need to initialize.");
    return configPda;
  }
  ...
  console.log("Preparing to initialize config...");
  try {
    const tx = new Transaction().add(ix);
    
    // Use our retry function
    await sendTransactionWithRetry(tx, [payer]);
    
    // Verify the config was created (with a short delay to allow for finalization)
