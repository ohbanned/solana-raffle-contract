...
// Helper to convert a number to a little-endian byte array (matching Rust's to_le_bytes)
function toLEBytes(value, byteLength = 8) {
...
async function createRaffle() {
  ...
  const { context, value: clock } = await connection.getLatestBlockhashAndContext();
  const slot = context.slot;
  const { timestamp } = await connection.getBlockTime(slot);
  
  console.log("Title:", title);
  console.log("Duration:", duration, "seconds");
  console.log("Current time from blockchain:", timestamp);
  
  // Prepare the title buffer exactly as it would be in the contract
  const titleBuffer = Buffer.alloc(32);
  Buffer.from(title).copy(titleBuffer, 0, 0, Math.min(title.length, 32));
  
  // Convert timestamp to little-endian bytes exactly as Rust does
  const timestampBytes = toLEBytes(timestamp);
  
  // Find raffle PDA using the correct seeds format
