...
// Connect to devnet with more resilient settings
...
/**
 * Utility function for a more reliable connection operation
 */
...
async function createRaffle() {
  try {
    ...
    // First, we need to get the current blockchain time with retry
    const slot = await withRetry(() => connection.getSlot('confirmed'));
    console.log("Current slot:", slot);
    
    const timestamp = await withRetry(() => connection.getBlockTime(slot));
    ...
    // Verify the raffle account with retry
    const raffleAccount = await withRetry(() => 
      connection.getAccountInfo(rafflePDA)
    );
